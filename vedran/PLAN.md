...
7. **Dodati nove testove u  `tests/storage.test.ts` i izvršiti provere iz sledećeg odeljka.** Buduće ručne izmene ograničiti na `src/main.ts`, novi `src/progress.ts` i `tests/storage.test.ts`. Ne menjati postojeće testove čuvanja, README.md ili ostale fajlove. `dist/` sme biti regenerisan postojećom komandom, ali ga ne uređivati ručno.
